...
class TabMNConf:
    ...
    def findSpath(self, ctrl, starting_spath, spath_count):
        ...
        printOK(f"Bip32 scan complete. result={self.spath_found}   spath={spath}")
    ...
    def findSpath_done(self):
        ...
        if self.spath_found:
            ...
            mess = f"Found address {currAdd} in HW account {currHwAcc} with spath_id {spath}"
            myPopUp_sb(self.caller, "info", 'SPMT - spath search', f"{mess}")
            self.ui.edt_spath.setValue(spath)
            self.findPubKey()

        else:
            mess = f"Scanned addresses <b>{starting_spath}</b> to <b>{starting_spath + spath_count - 1}</b> of HW account <b>{currHwAcct}</b>.<br>"
